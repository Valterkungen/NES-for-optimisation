...
class TriMesh(object):
  ...
  bny_spa    =  15;                   # Background elements in y (space)
...
# minTriArea = (dx+dy)/10000.;        # Minimum triangle size
  ...
  def loadMesh(self,n,yIce,alpha,beta): 
    ...
    for i in range(self.bnx+1):
      ...
      buffer3 = np.linspace(yice[i], ytop[i], self.bny_spa+1)[1:]					
    ...
    self.upperVI=[];
    vi =0
    for j in range(self.bny_wat+1+self.bny_ice+self.bny_spa):
      for i in range(self.bnx+1):
        if (i==0):	     self.leftVI.append(vi);
        if (i==self.bnx): self.rightVI.append(vi);
        if (j==0):	     self.lowerVI.append(vi);
        if (j==self.bny_spa + self.bny_ice + self.bny_wat): self.upperVI.append(vi);
  ...
  def plotMesh(self):
    ...
    ax.triplot(xy[:,0],xy[:,1],self.elements,'bo-');
...
#   plt.set_ylabel('y',size=14,weight='bold')
  ...
  def plotSoln(self, solVec, xmS, ymS, xmQ, ymQ, title=""):
    ...
    ax.set_aspect('equal');
    xy = np.asarray(self.mesh.vertices);
    ax.triplot(xy[:,0],xy[:,1],self.mesh.elements,'b-',linewidth=0.1);
    cont = ax.tricontourf(self.mesh.dtri,solVec, alpha=0.8, cmap=cmap)
    ax.plot(xmS, ymS, 'bs', xmQ, ymQ, 'bs')				
    fig.colorbar(cont)
    plt.tight_layout()
